...
启用后，Skill 会把当前技能机制拆成事件图和边界清晰的任务包，由制作 Agent 完成所选阶段，再由独立复核 Agent 检查资源、代码、测试和验收证据。
...
> 使用 `$creating-game-combat-vfx` 为剑、火、水三组技能设计完整战斗特效（PS：这里需要提供你自己的技能设计）。
...
如果这套 Skill 帮助你更稳定地完成战斗特效设计、资源制作、游戏接入或验收，欢迎 Star、Fork 或参与改进：

- **Star：** 方便以后重新找到，也帮助更多游戏开发者发现这个项目。
- **Fork：** 根据自己的引擎、渲染方式、美术管线或 Agent 工作流进行定制。
- **Issue：** 反馈使用问题、失败案例、规则缺口或兼容性问题。
- **Pull Request：** 提交经过验证的流程改进、工具修复、测试案例或文档补充。

Star 不是使用本项目的前提。建议先试用并检查生成资源、参考素材、第三方依赖以及目标平台的版权和性能要求，再决定是否将它纳入正式制作流程。

## License

本项目采用 [MIT License](LICENSE)。在保留原始版权声明和许可证文本的前提下，你可以使用、修改和分发本项目，也可以将其用于商业项目。

本项目提供的是战斗特效设计与制作流程、辅助工具和参考实现，不保证生成内容一定符合特定项目的美术标准、性能预算、平台规范或上线要求。使用者仍需自行完成资源授权检查、真机测试和最终验收。
