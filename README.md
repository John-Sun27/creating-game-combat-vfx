...
直接用浏览器打开 `tools/vfx-preview/index.html`，然后在页面中选择包含特效清单和 PNG 资源的目录。项目的构建或导出流程应自动在运行时资源旁生成 `effect-manifest.preview.json`，无需手动复制或维护第二份配置。无需安装依赖、启动服务器或联网；预览器会隔离显示缺失资源等问题，不影响同一目录中其他有效特效的播放。
